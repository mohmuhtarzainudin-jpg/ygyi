...
const App: React.FC = () => {
  // User Management State (kept for App-level reasons, but SettingsScreen manages users itself)
  ...
  useEffect(() => {
    ...
    const unsubTables = onSnapshot(query(collection(db, `stores/${storeId}/tables`), orderBy('name')), (snap) => {
      const latest = snap.docs.map(d => ({ id: d.id, ...(d.data() as any) } as Table));
    ...
    const unsubOperators = onSnapshot(collection(db, `stores/${storeId}/operators`), (snap) => {
      setOperators(snap.docs.map(d => ({ id: d.id, ...(d.data() as any) } as Operator)));
    }, handleSnapshotError);
    ...
    const unsubTrans = onSnapshot(qTrans, (snap) => {
      setTransactions(snap.docs.map(d => ({ id: d.id, ...(d.data() as any) } as Transaction)));
    }, handleSnapshotError);
    ...
    <>
      ...
      <div className="p-4 bg-slate-800 border-t border-slate-700">
        ...
        <button
          ...
          className={`w-full bg-accent hover:bg-emerald-600 disabled:bg-slate-700 disabled:text-slate-500 disabled:cursor-not-allowed text-primary font-bold py-3 rounded-lg text-lg transition shadow-lg`}
        >
          Bayar
        </button>
      </div>
    ...
    <div className="flex h-screen overflow-hidden bg-primary font-sans">
      ...
      <aside className="hidden md:flex w-20 lg:w-64 bg-secondary border-r border-slate-800 flex-col justify-between shrink-0 transition-all">
        <div>
          <div className="p-4 md:p-6 flex items-center justify-center lg:justify-start gap-3">
            <div className="w-10 h-10 rounded-full bg-accent flex items-center justify-center font-bold text-primary text-xl shadow-lg">Z</div>
      ...
      {(activeTab === 'billiard' || activeTab === 'cafe') && cart.length > 0 && (
        <button
          ...
          className="lg:hidden fixed bottom-20 right-4 bg-accent text-primary p-4 rounded-full shadow-lg z-30 flex items-center gap-2 animate-bounce-short"
        >
...
const SidebarItem: React.FC<SidebarItemProps> = ({ icon, label, active, onClick, badge }) => (
  <button
    onClick={onClick}
    className={`flex flex-col lg:flex-row items-center gap-1 lg:gap-3 p-3 rounded-lg transition w-full justify-center lg:justify-start relative ${active ? 'bg-accent text-primary font-bold shadow-lg' : 'hover:bg-slate-800'}`}
  >
    {icon}
              ...
              <div className="grid grid-cols-2 gap-2 mb-2">
                {operators.map(op => (
                  <button
                    ...
                    className={`p-2 rounded border text-sm font-bold ${operatorName === op.name ? 'bg-accent text-primary border-accent' : 'bg-slate-900 border-slate-700 text-slate-400 hover:bg-slate-800'}`}
                  >
                    {op.name}
                  </button>
                ))}
      ...
      <div className="mt-4">
        {isOccupied ? (
          <div className="grid grid-cols-3 gap-2">
            <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); onTopup(); }} className="bg-blue-600 hover:bg-blue-700 text-white py-3 md:py-2 rounded flex flex-col items-center justify-center text-sm">
              <Clock size={16} /> Topup
            </button>
            <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); onMove(); }} className="bg-orange-600 hover:bg-orange-700 text-white py-3 md:py-2 rounded flex flex-col items-center justify-center text-sm">
              <ArrowRightLeft size={16} /> Pindah
            </button>
            <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); onStop(); }} className="bg-red-600 hover:bg-red-700 text-white py-3 md:py-2 rounded flex flex-col items-center justify-center text-sm">
              <Power size={16} /> Stop
            </button>
          </div>
        ) : (
...
const StopTableModal: React.FC<{ storeId: string, table: Table, onClose: () => void }> = ({ storeId, table, onClose }) => {
  ...
  const handleConfirmStop = async () => {
    ...
    try {
      ...
      try {
        // fallback: try to use table's remoteOff if provided, else use numeric endpoint
        await controlLampForTable(table, 'off');
      } catch (e) {
...
const MoveTableModal: React.FC<{ storeId: string, fromTable: Table, tables: Table[], onClose: () => void }> = ({ storeId, fromTable, tables, onClose }) => {
  ...
  const handleMove = async (toTableId: string) => {
    ...
    try {
      // Strictly converting values to primitives to avoid undefined errors
      const moveData = {
        status: 'occupied',
          ...
          <div className="grid grid-cols-2 gap-3">
            {availableTables.map(t => (
              <button
                ...
                disabled={loading}
                className="bg-slate-800 hover:bg-slate-700 border border-slate-600 p-4 rounded text-white active:scale-95 transition disabled:opacity-50 disabled:cursor-not-allowed flex flex-col items-start"
              >
  ...
  return (
    <div>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div></div>
        <div className="w-full md:w-auto flex gap-2 overflow-x-auto pb-2">
          {categories.map(cat => (
            <button
              key={cat}
              onClick={() => setSelectedCategory(cat)}
              className={`px-4 py-2 rounded-full text-sm font-bold capitalize whitespace-nowrap transition flex-shrink-0 ${selectedCategory === cat ? 'bg-accent text-primary' : 'bg-secondary text-slate-400'}`}
            >
              {cat}
            </button>
          ))}
...
const InventoryScreen: React.FC<{ storeId: string, products: Product[], ingredients: Ingredient[] }> = ({ storeId, products, ingredients }) => {
  ...
  return (
    <div>
      ...
      <div className="flex gap-4 border-b border-slate-700 mb-6 overflow-x-auto pb-1">
        <button onClick={() => setActiveSubTab('products')} className={`pb-2 px-2 font-bold whitespace-nowrap ${activeSubTab === 'products' ? 'text-accent border-b-2 border-accent' : 'text-slate-400'}`}>Produk</button>
        <button onClick={() => setActiveSubTab('ingredients')} className={`pb-2 px-2 font-bold whitespace-nowrap ${activeSubTab === 'ingredients' ? 'text-accent border-b-2 border-accent' : 'text-slate-400'}`}>Bahan</button>
        ...
        <button onClick={() => setActiveSubTab('audit')} className={`pb-2 px-2 font-bold whitespace-nowrap ${activeSubTab === 'audit' ? 'text-accent border-b-2 border-accent' : 'text-slate-400'}`}>Audit</button>
      </div>
...
const AddProductModal: React.FC<{ storeId: string, onClose: () => void, ingredients: Ingredient[], editingProduct: Product | null }> = ({ storeId, onClose, ingredients, editingProduct }) => {
  ...
  const [isVariant, setIsVariant] = useState<boolean>(editingProduct?.isVariant || false);
  const [variants, setVariants] = useState<Variant[]>(editingProduct?.variants || []);
  const [isRecipe, setIsRecipe] = useState<boolean>(editingProduct?.isRecipe || false);
...
const SettingsScreen: React.FC<{ storeId: string, users: User[], operators: Operator[] }> = ({ storeId, users, operators }) => {
  ...
  useEffect(() => {
    if (editUserLocal) {
      setEditUserName(editUserLocal.name);
      setEditUserPin(editUserLocal.pin);
      setEditUserRole(editUserLocal.role);
    }
  }, [editUserLocal]);
  ...
  const handleEditUser = async () => {
    if (!editUserLocal || !editUserName || !editUserPin) return alert('Nama dan PIN wajib diisi');
    try {
      await setDoc(doc(db, `stores/${storeId}/users`, editUserLocal.id), {
        name: editUserName,
        pin: editUserPin,
        role: editUserRole
      });
      setEditUserLocal(null);
    } catch (e) {
      console.error(e);
  ...
  const handleDeleteUser = async () => {
    if (!deleteUserLocal) return;
    try {
      await deleteDoc(doc(db, `stores/${storeId}/users`, deleteUserLocal.id));
      setDeleteUserLocal(null);
    } catch (e) {
      console.error(e);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-secondary p-6 rounded-xl border border-slate-700 h-fit lg:col-span-2">
          ...
          <div className="mt-6 flex justify-end">
            <button onClick={handleSaveSettings} disabled={loadingSettings} className="bg-accent hover:bg-emerald-600 text-primary font-bold px-6 py-2 rounded flex items-center gap-2">
        ...
        <div className="bg-secondary p-6 rounded-xl border border-slate-700 h-fit">
          ...
          <div className="space-y-3">
            {users.map(u => (
              <div key={u.id} className="flex justify-between items-center bg-slate-800 p-3 rounded">
                ...
                <div className="flex gap-2">
                  <button onClick={() => { setEditUserLocal(u); }} className="bg-blue-600 hover:bg-blue-700 text-white px-2 py-1 rounded text-xs">Edit</button>
                  <button onClick={() => { setDeleteUserLocal(u); }} className="bg-red-600 hover:bg-red-700 text-white px-2 py-1 rounded text-xs">Hapus</button>
                </div>
              </div>
            ))}
          </div>
          ...
          {/* Edit Modal */}
          {editUserLocal && (
            <Modal title="Edit Akun Login" onClose={() => setEditUserLocal(null)}>
              <div className="space-y-3">
                ...
                <div className="flex gap-2 mt-2">
                  <button onClick={() => setEditUserLocal(null)} className="flex-1 bg-slate-700 hover:bg-slate-600 text-white py-2 rounded">Batal</button>
          ...
          {/* Delete Modal */}
          {deleteUserLocal && (
            <Modal title="Hapus Akun Login" onClose={() => setDeleteUserLocal(null)}>
              <div className="space-y-4">
                <p className="text-slate-300">Anda yakin ingin menghapus akun <span className="text-white font-bold">{deleteUserLocal.name}</span>?</p>
                <div className="flex gap-2">
                  <button onClick={() => setDeleteUserLocal(null)} className="flex-1 bg-slate-700 hover:bg-slate-600 text-white py-2 rounded">Batal</button>
